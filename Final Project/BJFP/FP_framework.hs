-- CS 440, Spring 2019 BJ Bloemker A20372354
...
--
-- DONE psp string -- ("Parse, solve print"): Parse string as a problem,
...
--
-- DONE parse_solve string -- similar to psp string but returns the
--     substitution data structure (instead of pretty printing it).
--

-- Main internal functions (include in Final Project)
--
-- DONE problem string -- parse string as a unification problem with
...
--
-- DONE unify equations -- solve a list of equations through unification and
...
psp input = case problem input of -- try parsing a problem
    ...
    Just(equations, _) -> case unify equations of
        ...
        Just substitutions -> ------ pretty print substitutions -----
            let
                pretty = beautify (stripOnlyJust (unify equations)) []
            in
                (sublist 0 (length pretty - 2) pretty) ++ "}"
            --beautify substitutions [] -- <- stub code


beautify [] out = concat (["{"] ++ [out])
beautify substitutions out = 
    let
        member = (substitutions !! 0)
        newSubst = drop 1 substitutions 
        var = fst member
        term = snd member
    in
        beautify newSubst ( var ++ "=" ++ (stripTerm (term)) ++ ", " ++ out)


stripTerm term =
    if isVar term then prettyVar term
    else if isID term then prettyID term
    else if isConst term then show (prettyConst term)
    else prettyFcn term
...
problem input =
    let
        call = (problem2 input [])
    in
        if "_FAILURE" `isInfixOf` (show call) then Nothing 
        else Just (call, "")

--problem p1   --> [ (VAR X, VAR Y), (VAR X, CONST 3) ] 
problem2 "" out = out
problem2 input out =  
    let 
        callOnInput = termFromProb input    -- > termFromProb p1         : Just ((Just (VAR "X",""),Just (VAR "Y","")),"X=3")
        simpleReturnFromCall = justToEquation callOnInput -- >           : (VAR "X",VAR "Y")       
    in
        (problem2 (justGetRemainder callOnInput) (out ++ [simpleReturnFromCall]))
----------------------------------------------------------------------
...
term_Func s =
    ...
    in
        if balance s == False then Just (VAR "_FAILURE", "")
        else if cleanArgs == "," then Just (VAR "_FAILURE", "")
        else Just (FCN functionName (map cleanTerms (map term (split cleanArgs))), (drop 1 remaining)) 


termFromProb s =
    let
        cleanString = stripBrackets (filter (\x -> x /= ' ') s)
        items = split cleanString
        firstItemSplitUp = split2 (items !! 0)
        loudTerm1 = (gabber cleanString) !! 0
        loudTerm2 = (gabber (sublist (justToInt (elemIndex '=' cleanString)+1) (length cleanString) cleanString)) !! 0
    in
        if ('(' `elem` (items !! 0)) == True 
            then Just ((loudTermToNorm(term_Func (loudTerm1)), --stripEqnJustToList
                        loudTermToNorm(term_Func (loudTerm2)) ),
                        stripComma (sublist (length loudTerm1 + length loudTerm2 + 1) (length cleanString) cleanString))
        else Just ( ( loudTermToNorm(term (last (reverse firstItemSplitUp))) , loudTermToNorm(term (last firstItemSplitUp)) )     , intercalate "," (drop 1 items))
...
term_VarOrID s =
    ...
    in 
        if isVariableName (firstItem) == True then Just (VAR firstItem, intercalate "," lastItems)
        else if isIdentifierName (firstItem) == True then Just (ID firstItem, intercalate "," lastItems)
        else if isConstant (firstItem) == True then Just ((CONST (read firstItem :: Int)), intercalate "," lastItems)
        else Just (VAR "_FAILURE", "")
...
unify equations = 
    let 
        call = unify2 equations []
        passable = convertT1toVar call []
    in
        if call == [] then Nothing
        else Just passable -- ((convertT1toVar (call)))
--result is Just [substitutions] i.e. Just [(VAR "Y", CONST 3), (VAR "X", CONST 3)]

unify2 [] solutions = solutions
unify2 problems solutions = -- P and S lists
    let 
        member = (problems !! 0)
        newP = drop 1 problems 
        t = getEquationFirstTerm member
        u = getEquationSecondTerm member
    in
        if (isVar u && not (isVar t)) then unify2 ([(u,t)] ++ newP) solutions -- add switched form to problems

        else if (isConst t || isID t) && (t == u) then unify2 newP solutions --Skip
        else if (isConst t || isID t) && (t /= u) then [] --fail

        else if isFcn (t) && (getFcnID t == getFcnID u) && (length (getFcnArgs t) == length (getFcnArgs u)) then unify2 ((mergeArgs (getFcnArgs t) (getFcnArgs u)) ++ newP) solutions
        else if isFcn (t) && (getFcnID t /= getFcnID u) then []
        else if isFcn (t) && (length (getFcnArgs t) /= length (getFcnArgs u)) then [] 

        else if t == u then unify2 newP solutions --Skip
        else unify2 (substituteAll (stripVar t) u newP []) ([(t,u)] ++ substituteAll (stripVar t) u solutions [])--t is var1
...
--makes sure parens are balanced (simulating stack)
...
prettyFcn (FCN id args) = concat ([id] ++ (map stripTerm args))


justToInt (Just a) = a
justToInt _ = (-1)

loudTermToNorm (Just (simpleTerm, _)) = simpleTerm
simplifyLoudTerm (Just ((term1, term2), _)) =  ((loudTermToNorm term1),(loudTermToNorm term2))
justToEquation (Just ((term1, term2), _)) = (term1, term2)
justToEquationList (Just (terms, _)) = terms


getEquationFirstTerm (term1, _) = term1
getEquationSecondTerm (_, term2) = term2


mergeArgs [] [] = []
mergeArgs (x:l1) (y:l2) = [(x,y)] ++ (mergeArgs l1 l2)

stripVar (VAR a) = a 

substituteAll _ _ [] out = out
substituteAll var1 term1 eqts out = 
    let 
        member = (eqts !! 0)
        newEqts = drop 1 eqts
        t1 = getEquationFirstTerm member
        t2 = getEquationSecondTerm member 
        applyThis = substitute var1 term1
    in
        [(applyThis t1, applyThis t2)] ++ (substituteAll var1 term1 newEqts out)


convertT1toVar [] out = out
convertT1toVar solutions out =
    let 
        member = (solutions !! 0)
        newSolns = drop 1 solutions
        t1 = getEquationFirstTerm member
        t2 = getEquationSecondTerm member 
    in
        [( (stripVar t1) :: Variable, t2) :: Subst] ++ (convertT1toVar newSolns out) 
...
stripBrackets s = 
    let
        sub' = drop 1 s
        sub = reverse (drop 1 (reverse sub'))
    in
        if take 1 s == "{" && take 1 (reverse s) == "}" then sub
        else s

--strip comma if it exist
stripComma "" = ""
stripComma s =
    if (s !! 0) == ',' then drop 1 s
    else s

--has a remainder
justHasRemainder (Just (_, "")) = False
justHasRemainder (Just (_, a)) = True
justHasRemainder _ = False

--get remainder
justGetRemainder (Just (_, a)) = a
justGetRemainder _ = ""

--sublist (substring)
sublist start end list = take (end - start) (drop start list)  --sublist
sublist2 list start end = take (end - start) (drop start list) --substr
...
--split at = [equals sign] (splitEquals)
split2 = _split2 []
_split2 ts "" = ts
_split2 ts s = _split2 (ts ++ [(token2 s)]) (drop (length(token2 s) + 1) s)
