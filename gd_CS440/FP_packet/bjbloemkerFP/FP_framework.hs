...
psp input = case problem input of -- try parsing a problem
    ...
    Just(equations, _) -> case unify equations of
        ...
        Just substitutions -> ------ pretty print substitutions -----
            let
                passable = stripOnlyJust (unify equations)
                pretty = beautify passable []
            in
...
termFromProb s =
    let
        cleanString' = filter (\x -> x /= ' ') s
        cleanString = stripBrackets cleanString'
        items = split cleanString
        firstItem = items !! 0
        lastItems = drop 1 items
        firstItemSplitUp = split2 firstItem
        firstItemIsFunction = '(' `elem` firstItem
    ...
    in
        if firstItemIsFunction == True 
            then Just ((loudTermToNorm(term_Func (loudTerm1)),
                        loudTermToNorm(term_Func (loudTerm2)) ),
                        stripComma (sublist (length loudTerm1 + length loudTerm2 + 1) (length cleanString) cleanString))
        else Just (    ( loudTermToNorm (term (last (reverse firstItemSplitUp))) , loudTermToNorm(term (last firstItemSplitUp)) )     , intercalate "," lastItems)
